...
{ 
  ...
  void PutDifference(PImage background)
  {
    PImOps.Copy(imgDiff,  Difference);
    imgDiff.loadPixels();
    background.loadPixels();
    for(int i= 0; i<background.width * background.height; i++){
     if(imgDiff.pixels[i] == color(0,0,0)){
        imgDiff.pixels[i] = background.pixels[i]; 
     }
    }
